<template>
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <div>
                <router-link :to="create" class="btn btn-primary btn-sm">Создать</router-link>
                <button class="btn btn-default btn-sm" @click="showFilter = !showFilter">Фильтр</button>
            </div>
        </div>
            ...
            <div class="filter" v-if="showFilter">
                ...
                <div class="filter-input">
                    <input type="text" class="form-control" v-model="params.search_query_1"
                           @keyup.enter="fetchData" placeholder="">
                </div>
                ...
                <div class="filter-btn">
                    <button class="btn btn-primary btn-sm btn-block" @click="fetchData">Фильтровать</button>
                </div>
            </div>
        ...
        <div class="panel-footer pagination-footer">
            ...
            <div class="pagination-item">
                <small>Записи {{model.from}} - {{model.to}} из {{model.total}}</small>
            </div>
            <div class="pagination-item">
                <small>Стр.: </small>
                <input type="text" class="pagination-input" v-model="params.page"
                       @keyup.enter="fetchData">
                <small> из {{model.last_page}}</small>
            </div>
            <div class="pagination-item">
                <button @click="prev" class="btn btn-default btn-sm">Пред.</button>
                <button @click="next" class="btn btn-default btn-sm">След.</button>
            </div>
        </div>
    </div>
</template>
